	...
	const items: PortfolioItem[] = [
		{
			id: 1,
			title: 'Корпус дрона',
			subtitle: 'Аерокосмічний прототип',
			category: 'proto',
			material: 'PETG',
			time: '36г',
			color: '#ff4d1c',
		...
		{
			id: 3,
			title: 'Статуетка лева',
			subtitle: 'Корпоративний подарунок',
			category: 'custom',
			material: 'PLA Silk',
			time: '18г',
			color: '#00d4ff',
...
<main>
	...
	<div class="portfolio-grid">
		{#each filtered as item, i (item.id)}
			{@const delay = (i % 6) * 60}
			<article class="portfolio-card" style="animation-delay:{delay}ms">
				<!-- color accent bar -->
